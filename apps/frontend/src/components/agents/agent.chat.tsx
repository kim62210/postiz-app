  ...
  const convertContentToImagesAndVideo = useMemo(() => {
    return (props.message?.content || '')
      .replace(/Video: (http.*mp4\n)/g, (match, p1) => {
        return `<video controls class="h-[150px] w-[150px] rounded-[8px] mb-[10px]"><source src="${p1.trim()}" type="video/mp4">${t('agents_video_fallback', 'Your browser does not support the video tag.')}</video>`;
      })
      .replace(/Image: (http.*\n)/g, (match, p1) => {
